...
from kext import K_ext, K_ext_combination, GPRwDO
...
yext = tf.concat([energy_train, tf.reshape(grad_train, (grad_train.shape[0], -1))], axis=1)
yext = tf.reshape(tf.transpose(yext), (yext.shape[0]*yext.shape[1], 1))

descriptor = get_descriptor(ani=True, n_atm_types=n_atm_types)

# gather Jacobian
n = 1
for i in range(int((nconf/nskip)/n)):
    crds = coords_train[i*n:(i+1)*n]
    print(crds.shape)
    atom_types1 = tf.repeat(tf.expand_dims(atom_types_vec, axis=0), len(crds), axis=0)
    crd1_solu = crds[:,:nsolu]
    crd1_solv = crds[:,nsolu:]
    with tf.GradientTape(persistent=False, watch_accessed_variables=False) as tape:
        tape.watch(crd1_solu)
        crd1 = tf.concat((crd1_solu, crd1_solv), axis=1)
        X = tf.reshape(descriptor(crd1, atom_types1), (n, -1))
    dxdc1 = tape.batch_jacobian(X,crd1_solu)
    if i == 0: 
        grads1 = dxdc1
        Xin = X
    else:
        grads1 = tf.concat((grads1, dxdc1), 0)
        Xin = tf.concat((Xin, X), 0)

grads1 = tf.reshape(grads1, (grads1.shape[0], grads1.shape[1], -1))

# sss kernel
ka = []
num_kernels = nsolu
for i in range(num_kernels):
    ki = K_ext(nsolu,atom_types_vec,active_dims=[j+(nfeat*i) for j in range(nfeat)])
    ka.append(ki)
kernel = K_ext_combination(ka)

data = (coords_train, yext) 
model = GPRwDO(data, descriptor,atom_types_vec,kernel, grads1, Xin)

opt = gpflow.optimizers.Scipy()
opt.minimize(model.training_loss, model.trainable_variables)

# Make Predictions
K = model.kernel.K(descriptor, atom_types1, coords_train, grads1, Xin)
k_diag = tf.linalg.diag_part(K)
s_diag_obs = tf.fill(tf.expand_dims(tf.shape(coords_train)[0],-1), model.likelihood.variance)
s_diag_der = tf.fill((tf.shape(k_diag)-tf.shape(coords_train)[0]), model.likelihood.der_variance)

s_diag = tf.concat((s_diag_obs, s_diag_der), 0)
K = tf.linalg.set_diag(K, k_diag + s_diag)

kinv = tf.linalg.inv(K)
alpha = tf.matmul(kinv,yext)
...
for i in range(nconf):
    ...
    with tf.GradientTape(persistent=False) as tape:
        ...
        ks = model.kernel.Ks(descriptor, crds1, coords_train, grads1, Xs1, Xin)
...
print(np.argwhere(tf.math.is_nan(preds).numpy()))
print(tf.math.reduce_any(tf.math.is_nan(preds_f)))
print(np.argwhere(tf.math.is_nan(preds_f).numpy()))
...
for kk in model.kernel.ka:
    x0.append(kk.lengthscales.numpy())
    x0.append(kk.variance.numpy())
# Add the likelihood parameters
x0.append(model.likelihood.der_variance.numpy())
x0.append(model.likelihood.variance.numpy())

class Global_Bounds:
    #def __init__(self, xmax=[100.0,100.0,50.0,50.0], xmin=[0.1,0.1,0.01,0.01] ):
    def __init__(self, xmax=[1000.0 for i in range(2*nsolu+2)], xmin=[0.01 for i in range(2*nsolu+2)]):
...
#local_bounds=((0.01,1000.0),(0.01,1000.0),(0.01,100.0),(0.01,100.0))
...
def optimize_with_basinhopping(model, initial_params, bounds, accept_test, n_iterations=100):
    ...
    def objective_func_wrapper(params):
        ...
        for i in range(nsolu):
            model.kernel.ka[i].lengthscales.assign(params[j])
            j += 1
            model.kernel.ka[i].variance.assign(params[j])
            j += 1
        model.likelihood.der_variance.assign(params[j])
        j += 1
        model.likelihood.variance.assign(params[j])

        #with tf.GradientTape() as tape_hess:
        #    tape_hess.watch(model.trainable_variables)
        ...
        #hess = tape_hess.gradient(gradients, model.trainable_variables)
        #loss, gradients, hess = tf.cast(loss, dtype=tf.float64), tf.cast(gradients, dtype=tf.float64), tf.cast(hess, dtype=tf.float64)
    ...
    def minimizer(params):
        #loss, gradients, hess = objective_func_wrapper(params)
...
print(x_opt, loss)
#x_opt = tf.cast(x_opt, dtype=tf.float32)

# Loop through all kernels in the list
j = 0
for i in range(nsolu):
    model.kernel.ka[i].lengthscales.assign(x_opt[j])
    j += 1
    model.kernel.ka[i].variance.assign(x_opt[j])
    j += 1
model.likelihood.der_variance.assign(x_opt[j])
j += 1
...
with open('params_kext_rbf_atom_f64.pkl', 'wb') as f:
...
for i in range(nconf):
    ...
    with tf.GradientTape(persistent=False) as tape:
        ...
        ks = model.kernel.Ks(descriptor, crds1, coords_train, grads1, Xs1, Xin)
        preds_ = tf.matmul(ks, alpha)
    preds_f_ = 1 * tape.gradient(preds_, crds1)
    if i == 0:
        preds = preds_
        preds_f = preds_f_
    else:
...
np.savez_compressed('const_kext_rbf', alpha=alpha.numpy(), X=Xin.numpy(), Y=yext.numpy(), grads=grads1.numpy())
