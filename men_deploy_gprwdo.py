# Deploy GPRwDO to correct AM1/MM for RESD scan of Menshutkin reaction
#
...
path = './'
npz = np.load(f'{path}/const_kext_rbf.npz')
alpha = Tensor(npz['alpha'], dtype=ddtype)
grads = Tensor(npz['grads'], dtype=ddtype)
Xin = Tensor(npz['X'], dtype=ddtype)
...
energy_train = tf.expand_dims(energy_test[::nskip], -1)
...
atom_types2 = tf.repeat(tf.expand_dims(atom_types_vec, axis=0), len(coords_train), axis=0)
X2 = descriptor(coords_train, atom_types2)
...
model = GPRwDO(data, descriptor,atom_types_vec,kernel, grads, Xin) 

with open('./params_kext_rbf_atom_f64.pkl', 'rb') as f:
...
stream.charmm_script(f'open write unit 22 file name men_deploy_gprwdo.dcd')
...
def e_gpr(natoms,
    ...
    with tf.GradientTape(
        ...
        ks = model.kernel.Ks1(descriptor, crds, X2, grads, Xs)
...
ax.plot(rc, sqm_energy, 'b', label='AM1-GPRwDO/MM')
...
fig.savefig("men_deploy_gprwdo.png")
plt.show()
